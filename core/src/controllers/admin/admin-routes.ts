...
function mountAdminRoutes(app: Application, ctx: AdminContext): void {
    ...
    // 获取设备预设列表
    app.get('/api/admin/device-presets', authRequired, requirePermission('system:*'), (_req: Request, res: Response) => {
        try {
    ...
    // 获取系统配置
    app.get('/api/admin/system-config', authRequired, requirePermission('system:*'), (_req: Request, res: Response) => {
        try {
    ...
    // 保存系统配置
    app.post('/api/admin/system-config', authRequired, requirePermission('system:*'), (req: Request, res: Response) => {
        try {
    ...
    // 重置系统配置为默认值
    app.post('/api/admin/system-config/reset', authRequired, requirePermission('system:*'), (req: Request, res: Response) => {
        try {
            const defaultConfig = getDefaultSystemConfig();
    ...
    // 导出数据备份
    app.get('/api/admin/backup/export', authRequired, requirePermission('system:*'), (_req: Request, res: Response) => {
        try {
            ensureDataDir();
    ...
    // 导入数据备份
    app.post('/api/admin/backup/import', authRequired, requirePermission('system:*'), (req: Request, res: Response) => {
        try {
    ...
    // ============ 清理工具 API（仅管理员） ============
    // 系统清理工具
    app.post('/api/admin/cleanup', authRequired, requirePermission('system:*'), (req: Request, res: Response) => {
        try {
